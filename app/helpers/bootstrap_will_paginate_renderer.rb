class BootstrapWillPaginateRenderer < WillPaginate::ActionView::LinkRenderer
  ...
  def previous_or_next_page(page, text, classname)
    if page
      tag(:li, link(text.include?('Next') ? 'Next' : 'Back', page, class: 'page-link'), class: "page-item #{classname}")
    else
      tag(:li, link(text.include?('Next') ? 'Next' : 'Back', '#', class: 'page-link'), class: "page-item disabled")
    end
  end
end